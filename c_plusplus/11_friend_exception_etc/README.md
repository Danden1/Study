...
예외가 잡히는 곳까지 도달하기 위해, 프로그램은 스택에 있는 자동 기억공간 변수들을 해체함



### 그 밖의 기능

예외 지정자나 catch 블록이 참조 지정해도, 컴파일런느 언제나 예외가 발생할 때 임시 복사본 만듬



## RTTI

runtime type identification 약자
...
- dynamic_cast연산자, 가능하면 기초 클래스형을 지시하는 포인터로부터 파생클래스 형을 지시하는 포인터 생성

불가능하면 널포인터 반환

- typeid 연산자는 어떤 객체의 정확한 데이터형을 식별하는 하나의 값 리턴

- type_info 구조체는 어떤 특별한 데이터형에 대한 정보 저장


RTTI는 가상함수들을 가지고 있는 클래스 계층에서만 사용 가능
...
그 객체의 주솔르 특정형의 포인터에 안전하게 대입할 수 있는 지 알려줌



파생 클래스는 객체의 주소를 기초 클래스 포인터에 대입하므로 안전.



데이터형 알려고 하는 이유는, 어떤 특별한 메서드 호출하는 것이 안전한지 알기 위해.



typeid, type_info

typeid는 두 객체의 데이터 형이 같은지 결정 가능.
...
typeid연산자는 type_info객체에 대한 참조 리턴

typeinfo는 typeinfo헤더파일에 있음
